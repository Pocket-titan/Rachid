...
const playStream = async (voice_channel, link, user_options = {}) => {
  ...
  const dispatcher = (await join_channel(voice_channel)).playStream(stream, { volume: volume / 100 });
...
const triggers = [
  ...
  {
    q: 'rachid kom',
    a: (voice_channel, q) => playFile(voice_channel, 'ikbenrachid.mp3', { seek: 51, volume: 0.03 }),
    type: 'voice',
  },
  {
    q: 'rachid heb je even voor mij',
    a: (voice_channel, q) => playFile(voice_channel, 'hebjeevenvoormij.mp3', { volume: 0.01 }),
    type: 'voice',
  },
  {
    q: 'rachid fatoe',
  ...
  },
  {
    ...
    a: (voice_channel, query) => {
      ...
      search(query.replace('rachid play', '').trim(), options, (err, results) => {
        ...
        playStream(voice_channel, results.filter((result) => result.kind === 'youtube#video')[0].link);
      });
    },
    type: 'search',
  },
